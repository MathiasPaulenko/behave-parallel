...
`behave-parallel` implements Behave's `ITestRunner` interface via `ParallelRunner`.
...
5. **Serial phase** — Runs `@serial` work units one at a time after all parallel
   workers complete.
6. **Updates timings** — Persists observed durations to `.behave-parallel-timing.json`
...
By default, `behave-parallel` uses Longest Processing Time (LPT) scheduling.
...
- [ParallelRunner](api-reference.md#behave_parallel.runner.ParallelRunner)
- [ConfigSnapshot](api-reference.md#behave_parallel.config.ConfigSnapshot)
- [WorkUnit](api-reference.md#behave_parallel.work_unit.WorkUnit)
- [WorkerResult](api-reference.md#behave_parallel.result.WorkerResult)
- [TimingStore](api-reference.md#behave_parallel.timing.TimingStore)
